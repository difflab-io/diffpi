...
6. Call `plan_start_execution` with inline/background mode and commit-per-phase/no-commit policy. For `/plan go --bg`, the already-spawned orchestrator must pass `coordinator: current`; normal callers omit it (default `spawn`). When an inline execution marks the plan completed, `plan_update_status` exits to the default mode automatically. Report the execution ID and whether the foreground mode changed.
