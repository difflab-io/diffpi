...
describe('review browser launch', () => {
  it('selects platform commands and falls back when launch fails', async () => {
    const url = 'https://github.com/difflab-io/diffpi/pull/1';
    expect(reviewBrowserCommand(url, 'darwin')).toEqual({ command: 'open', args: [url] });
    expect(reviewBrowserCommand(url, 'linux')).toEqual({ command: 'xdg-open', args: [url] });
    expect(reviewBrowserCommand(url, 'win32')).toEqual({ command: 'cmd', args: ['/c', 'start', '', url] });
    expect(
      await openReviewUrl(url, async () => {
...
describe('syncLocalThreadArtifact', () => {
  it('creates a missing local thread ledger on first sync', async () => {
    ...
    expect(content).toContain('**Recorded response**');
    expect(content).toContain('_No response recorded._');
    expect(parseThreadArtifact(content)[0]).toMatchObject({
      id: 'thread-1',
      resolved: false,
      addressed: false,
    });
